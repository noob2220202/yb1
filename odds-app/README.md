...
**배팅 금액을 사이트에서 유동적으로 조정**: 경기 상세 페이지 상단의 "총
스테이크(원)" 입력에 원하는 금액을 넣고 "다시 계산"을 누르면, 그 경기의 모든
조합 카드가 그 금액 기준으로 다시 계산된다(`/fixtures/{id}?total_stake=...`
쿼리 파라미터, 기본값은 `.env`의 `DEFAULT_TOTAL_STAKE`). 계산기 페이지는
원래부터 "총 예산 기준" 입력 모드로 금액을 자유롭게 넣을 수 있다. 저장 시
그 시점에 계산된 실제 스테이크 금액이 픽/텔레그램 메시지에 그대로 들어간다.

**픽 저장 시 코멘트 남기기**: 조합 카드/계산기 저장 폼 모두에 "코멘트" 입력칸이
있다. 채워서 저장하면 텔레그램 메시지에도 코멘트가 그대로 포함되고(예: "홈팀
주전 결장" 같은 메모), `/picks` 목록에도 함께 표시된다.
